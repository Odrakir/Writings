...
*/
//Service Locator implementation by Florian Gößler http://www.floriangoessler.de/ios/2016/01/24/Swift-Service-Locator.html
...
var user = loadUser(userId: "USERID")
/*:
...
*/
func loadUserCurrying(userId:String) -> (Service) -> User?
{
    return { service in
        let json = service.fetchUser(userId:userId)
        return User(json: json)
...
let loadUserAction = loadUserCurrying(userId: "0001")
...
*/
func loadUserReader(userId:String) -> Reader<Service, User?>
{
...
//We get the function that will load an specific user
let loadUserActionR = loadUserReader(userId: "0001")

//Sometime later we can inject the service to get the user
user = loadUserActionR.run(WebService())
/*:
...
let userFriends = loadUserReader(userId: "0001")
    .map { $0!.userId }
    .flatMap(loadFriends)
...
let getManyUsers = ["1", "2", "3", "4"].map(loadUserReader).map{ $0.run(WebService()) }
...
let loadUserAndFriendsAction = loadUserReader(userId: "0001").zip(loadFriends(userId: "0001")) { ($0, $1) }

//Finally we run the new Reader
let (_user, _friends) = loadUserAndFriendsAction.run(WebService())
/*:
